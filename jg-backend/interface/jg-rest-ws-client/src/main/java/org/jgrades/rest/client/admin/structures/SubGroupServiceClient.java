...
public class SubGroupServiceClient extends RestCrudPagingServiceClient<SubGroup, Long>
    ...
    public SubGroup getWithId(Long id) {
        ...
        ResponseEntity<SubGroup> response = restTemplate.exchange(backendBaseUrl + serviceUrl,
                HttpMethod.GET, HttpEntity.EMPTY, new ParameterizedTypeReference<SubGroup>() {
                });
        return response.getBody();
    }

    @Override
    public List<SubGroup> getWithIds(List<Long> ids) {
    ...
    public Page<SubGroup> getPage(Integer number, Integer size) {
        URI uri = UriComponentsBuilder.fromHttpUrl(backendBaseUrl + crudUrl)
                ...
                .build().encode().toUri();
        ResponseEntity<Page<SubGroup>> response = restTemplate.exchange(uri,
                HttpMethod.GET, HttpEntity.EMPTY, new ParameterizedTypeReference<Page<SubGroup>>() {
                });
        return response.getBody();
    }
}

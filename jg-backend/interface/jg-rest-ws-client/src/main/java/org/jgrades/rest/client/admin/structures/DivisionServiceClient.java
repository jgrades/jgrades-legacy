...
public class DivisionServiceClient extends RestCrudPagingServiceClient<Division, Long>
    ...
    public Division getWithId(Long id) {
        ...
        ResponseEntity<Division> response = restTemplate.exchange(backendBaseUrl + serviceUrl,
                HttpMethod.GET, HttpEntity.EMPTY, new ParameterizedTypeReference<Division>() {
                });
        return response.getBody();
    }

    @Override
    public List<Division> getWithIds(List<Long> ids) {
    ...
    public Page<Division> getPage(Integer number, Integer size) {
        URI uri = UriComponentsBuilder.fromHttpUrl(backendBaseUrl + crudUrl)
                ...
                .build().encode().toUri();
        ResponseEntity<Page<Division>> response = restTemplate.exchange(uri,
                HttpMethod.GET, HttpEntity.EMPTY, new ParameterizedTypeReference<Page<Division>>() {
                });
        return response.getBody();
    }
}
